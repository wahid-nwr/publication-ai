...
import io.wahid.publication.ai.util.TrendMath;
...
public class Neo4jWeatherRepository {
    ...
    public TrendResult computeTrend(List<YearValue> series) {
        if (series.size() < 2) {
            return new TrendResult(TrendDirection.FLAT, 0.0, series);
        }
        ...
        return new TrendResult(direction, slope, series);
    }
